...
public:
    ...
    void bind(int index, const T &value)
    {
        ...
        {
            sqlite3_bind_text(m_handle, index, value, -1, SQLITE_STATIC);
        }
        else if constexpr (std::is_same_v<std::string, paramType>)
        {
            sqlite3_bind_text(m_handle, index, value.data(), value.size(), SQLITE_STATIC);
        }
        else if constexpr (std::is_same_v<Blob, paramType>)
        {
            sqlite3_bind_blob(m_handle, index, value.data.data(), value.data.size(), SQLITE_STATIC);
        }
